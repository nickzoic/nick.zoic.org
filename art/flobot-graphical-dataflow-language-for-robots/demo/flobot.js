window.onload = function () {
    ...
    var nodes = [ 
        ...
        },
        {
            ...
            outputs: [ ],
            connect: [ [0,0], [2,0] ],
            geometry: { x: 175, y: 250 }
        }
    ];

    // DisplayPage

    function DisplayPage(json_data) {
        this.svg_element = document.createElementNS(svg_xmlns, 'svg');
        this.input_ports = new DisplayPorts(svg_element, json_data.inputs);
        this.output_ports = new DisplayPorts(svg_element, json_data.outputs);
        this.nodes = json_data.map(function (json_node) {
            return DisplayNode(json_node);
        });
        this.edges = json_data.map(function (json_node) {
            return json_node.connect.map(function (c) {
                return DisplayEdge(c);
            });
    ...
    function DisplayPorts(node_element, ports) {
    ...
    DisplayPorts.prototype.min_width = function () {
    ...
    DisplayPorts.prototype.set_width = function (width) {
